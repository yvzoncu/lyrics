...
@app.post("/api/new-fetch-and-process")
async def fetch_and_process(request: SongRequest = Body(...)):
    conn = get_db_connection()
    cursor = conn.cursor()
    try:
        ...
        cursor.execute(
            """
            SELECT id, song, artist, dominant_emotions FROM songs
            WHERE song = %s AND artist = %s
            """,
            (request.song, request.artist),
        )
        ...
        # Step 3 & 4: Process lyrics, calculate overall emotions, then insert song
        def process_lyrics():
            new_embeddings = []
            new_ids = []
            all_emotions = {}  # Dictionary to track emotion frequencies
            emotion_scores = {}  # Dictionary to accumulate emotion scores
            total_lines = 0
            ...
            for lyric in lyrics:
                if not lyric.strip():  # Skip empty lines
                    continue

                # Get emotion predictions for this line
                results = emotion_model(lyric)[0]
                significant_emotions = []

                # Filter emotions with score > 0.1 threshold
                for res in results:
                    if res["score"] > 0.1:
                        significant_emotions.append(res["label"])

                        # Accumulate emotion scores for overall calculation
                        if res["label"] in emotion_scores:
                            emotion_scores[res["label"]] += res["score"]
                        else:
                            emotion_scores[res["label"]] = res["score"]
                ...
                # Create embedding vector for this line
                emb = get_emotion_vector(lyric)
                total_lines += 1
                ...
                try:
                    cursor_inner.execute(
                        """
                        INSERT INTO lyrics (song_id, lyric, embedding, emotions)
                        VALUES (%s, %s, %s, %s)
                        RETURNING id
                        """,
                        (
                            song_id,
                            lyric,
                            psycopg2.Binary(pickle.dumps(emb)),
                            json.dumps(significant_emotions),
                        ),
                    )
                    lyric_id = cursor_inner.fetchone()["id"]
                    conn_inner.commit()
            ...
            # Calculate overall dominant emotions for the song
            dominant_emotions = []
            if total_lines > 0:
                # Calculate average score for each emotion
                for emotion, score in emotion_scores.items():
                    emotion_scores[emotion] = score / total_lines

                # Get top 3 emotions for the song by average score
                dominant_emotions = sorted(
                    emotion_scores.items(), key=lambda x: x[1], reverse=True
                )[:3]
                dominant_emotions = [emotion for emotion, _ in dominant_emotions]

            # Update song with dominant emotions
            ...
            try:
                cursor_update.execute(
                    """
                    UPDATE songs 
                    SET dominant_emotions = %s,
                        emotion_data = %s
                    WHERE id = %s
                    """,
                    (json.dumps(dominant_emotions), json.dumps(all_emotions), song_id),
                )
                conn_update.commit()
            finally:
                cursor_update.close()
            ...
            # Add to vector index if we have embeddings
            if new_embeddings:
                index.add(np.array(new_embeddings, dtype="float32"))
                lyric_ids.extend(new_ids)

            return len(new_ids), dominant_emotions, all_emotions

        # First insert song with placeholder for emotions
        cursor.execute(
            """
            INSERT INTO songs (song, artist, full_lyric, dominant_emotions)
            VALUES (%s, %s, %s, %s)
            RETURNING id
            """,
            (request.song, request.artist, full_lyrics, json.dumps([])),
        )
        song_id = cursor.fetchone()["id"]
        conn.commit()

        # Run the lyric processing in the background
        lyrics_count, dominant_emotions, emotion_data = (
            await asyncio.get_event_loop().run_in_executor(executor, process_lyrics)
        )
...
@app.post("/api/v2-fetch-and-process")
async def fetch_and_process(request: SongRequest = Body(...)):
    conn = get_db_connection()
    cursor = conn.cursor()
    try:
        ...
        # Process lyrics and calculate emotions
        def process_lyrics():
            new_embeddings = []
            all_emotions = {}  # Dictionary to track emotion frequencies
            emotion_scores = {}  # Dictionary to accumulate emotion scores
            total_lines = 0
            lyrics_with_emotions = []  # Store lyrics with their emotions
            ...
            for lyric in lyrics:
                ...
                # Skip if only "neutral" is above threshold
                if len(significant_emotions) == 1 and "neutral" in significant_emotions:
                    continue

                # Skip if no significant emotions found
                ...
                for res in results:
                    if res["score"] > 0.1:
                        ...
                        if emotion in emotion_scores:
                            emotion_scores[emotion] += res["score"]
                        else:
                            emotion_scores[emotion] = res["score"]
                ...
                # Store lyric with its emotions
                lyrics_with_emotions.append(
                    {"lyric": lyric, "emotions": significant_emotions}
                )
            ...
            return total_lines, dominant_emotions, all_emotions, lyrics_with_emotions

        # Run the lyric processing
        lyrics_count, dominant_emotions, emotion_data, lyrics_with_emotions = (
            await asyncio.get_event_loop().run_in_executor(executor, process_lyrics)
        )

        # Insert song with all data
        cursor.execute(
            """
            INSERT INTO songs (
                song, 
            ...
            (
                ...
                json.dumps(lyrics_with_emotions),
            ),
        )
        song_id = cursor.fetchone()["id"]
        conn.commit()
